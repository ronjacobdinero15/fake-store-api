...
function Footer({ subTotal }) {
  const { setCartedProducts } = useProducts()
  const totalPayment = (subTotal + DEFAULT_SHIPPING_FEE).toFixed(2)
  ...
  return (
    <footer className={styles.footer}>
      <div className={styles.paymentMethod}>
        ...
        <select onChange={e => Number(e.target.value)}>
          <option value="epayment">Credit / Debit Card</option>
          <option value="cod">Cash on Delivery</option>
        </select>
      </div>

      <span className={styles.totalDetails}>
        Shipping fee: <strong>${DEFAULT_SHIPPING_FEE}</strong>
      </span>
      <span className={styles.totalDetails}>
        Total: <strong>${totalPayment}</strong>
      </span>

      <Button type="placeOrder" onClick={handlePlaceOrder}>
        Place your order
        <svg
          width="30px"
          height="30px"
